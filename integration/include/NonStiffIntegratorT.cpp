...
NonStiffIntegratorT::NonStiffIntegratorT(HAI_function & my_func,
	      const int nin, NB_TYPE xin, NB_TYPE xendin, 
	      NB_TYPE dxin, int ntotin, int nrdensin, int itolerin, 
	      NB_TYPE *rtolerin, NB_TYPE *atolerin, const int ioutin, 
	      NB_TYPE hin, NB_TYPE hmaxin, int nmaxin, NB_TYPE uroundin, 
	      NB_TYPE safein, NB_TYPE faclin, NB_TYPE facrin, NB_TYPE betain, 
	      int nstiffin, unsigned *icontin) :
  IntegratorT(nin, xin, xendin, dxin, ntotin, itolerin, rtolerin, 
	      atolerin, ioutin, hin, hmaxin, nmaxin, uroundin, safein, faclin, 
	      facrin),
  Function(my_func),
  nrdens(nrdensin), beta(betain), meth(1), nstiff(nstiffin), icont(icontin)
{
  ...
  if (nrdens > n) {
    ...
    rcont1 = new NB_TYPE[nrdens];
    rcont2 = new NB_TYPE[nrdens];
    rcont3 = new NB_TYPE[nrdens];
    rcont4 = new NB_TYPE[nrdens];
    rcont5 = new NB_TYPE[nrdens];
...
void NonStiffIntegratorT::Integrate(NB_TYPE y[], // The initial condition
				    std::vector< std::vector<NB_TYPE> > &flot)
{

  int idid = CoreIntegrator(y, flot);
  ...
  if (iout != 0) {
    if ( flot.size() < ntot ) {
      ...
      for ( int i=0; i < n; i++ ) flot_t[i] = y[i];
...
NB_TYPE sign(NB_TYPE a, NB_TYPE b)
{
  return (b > 0.0) ? fabs(a) : -fabs(a);
} // sign

// calculates initial value for step length, h
NB_TYPE NonStiffIntegratorT::hinit( NB_TYPE y[] ) // the initial condition
{
  int iord = 5;
  NB_TYPE posneg = sign(1.0, xend-x);
  NB_TYPE sk, sqr;
  NB_TYPE dnf = 0.0;
  NB_TYPE dny = 0.0;
  NB_TYPE atoli = atoler[0];
  NB_TYPE rtoli = rtoler[0];
  ...
  NB_TYPE der12 = max(fabs(der2), sqrt(dnf));

  NB_TYPE h1;
  if (der12 <= (NB_TYPE)1.0e-15) h1 = max((NB_TYPE)1.0e-6, fabs(h)*(NB_TYPE)1.0e-3);
  else h1 = pow(0.01/der12, 1.0/(NB_TYPE)iord);
...
int NonStiffIntegratorT::CoreIntegrator(NB_TYPE y[], // the initial condition
				       std::vector<std::vector<NB_TYPE> > &flot)
{
  NB_TYPE c2, c3, c4, c5, e1, e3, e4, e5, e6, e7, d1, d3, d4, d5, d6, d7;
  NB_TYPE a21, a31, a32, a41, a42, a43, a51, a52, a53, a54;
  NB_TYPE a61, a62, a63, a64, a65, a71, a73, a74, a75, a76;
  ...
  NB_TYPE posneg = sign(1.0, xend-x);
  NB_TYPE facold = 1.0e-4;
  NB_TYPE expo1 = 0.2 - beta*0.75;
  NB_TYPE facc1 = 1.0/facl;
  NB_TYPE facc2 = 1.0/facr;
  ...
  while (true) {
    ...
    err = sqrt(err/(NB_TYPE)n);

    // computation of hnew
    NB_TYPE fac11 = pow(err, expo1);
    // Lund-stabilization
    NB_TYPE fac = fac11/pow(facold,beta);
    ...
    if (err <= 1.0) {
      // step accepted
      facold = max(err, (NB_TYPE)1.0e-4);
      naccpt++;

      // stiffness detection
      if (!(naccpt % nstiff) || (iasti > 0)) {
	NB_TYPE stnum = 0.0, stden = 0.0;
	for (int i = 0; i < n; i++) {
	  sqr = k2[i] - k6[i];
	  stnum += sqr*sqr;
	  sqr = yy1[i] - ysti[i];
	...
	if (nrdens == n)
      ...
      memcpy(k1, k2, n*sizeof(NB_TYPE));
      memcpy(y, yy1, n*sizeof(NB_TYPE));
      xold = x;
      x += h;
...
NB_TYPE NonStiffIntegratorT::ContinuousOutput(unsigned i)
{
  ...
  NB_TYPE theta = (xd - xold)/hold;
  NB_TYPE theta1 = 1.0 - theta;
